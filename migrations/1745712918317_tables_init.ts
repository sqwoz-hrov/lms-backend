...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema.createType('hr_connection_status')
    .asEnum(['waiting_us', 'waiting_hr', 'rejected', 'offer'])
    .execute();

  // Create interview type enum
  await db.schema.createType('interview_type')
    .asEnum(['screening', 'technical_interview', 'final', 'other'])
    .execute();

  // Create material type enum
  await db.schema.createType('material_type')
    .asEnum(['video', 'article', 'book', 'course', 'other'])
    .execute();
  ...
  await db.schema
    ...
    .addColumn('student_user_id', 'uuid', (col) => 
      col.notNull().references('user.id').onDelete('cascade')
    )
    .addColumn('mentor_user_id', 'uuid', (col) => 
      col.notNull().references('user.id').onDelete('cascade')
    )
  ...
  await db.schema
    ...
    .addColumn('hr_connection_id', 'uuid', (col) => 
      col.notNull().references('hr_connection.id').onDelete('cascade')
    )
    .addColumn('name', 'varchar(64)', (col) => col.notNull())
  ...
  await db.schema
    ...
    .addColumn('subject_id', 'uuid', (col) => 
      col.notNull().references('subject.id').onDelete('cascade')
    )
    .addColumn('name', 'varchar(64)', (col) => col.notNull())
    .addColumn('type', sql`material_type`, (col) => col.notNull())
    .addColumn('content_link', 'varchar(2048)')
    .addColumn('markdown_content_id', 'uuid', (col) => 
      col.references('markdown_content.id').onDelete('set null')
    )
    .execute();
  ...
  await db.schema
    ...
    .addColumn('contact_stage_id', 'uuid', (col) => 
      col.notNull().references('hr_connection.id').onDelete('cascade')
    )
  ...
  await db.schema
    .createIndex('task_student_user_id_idx')
    .on('task')
    .column('student_user_id')
    .execute();

  await db.schema
    .createIndex('task_mentor_user_id_idx')
    .on('task')
  ...
  await db.schema
    .createIndex('call_student_user_id_idx')
    .on('call')
    .column('student_user_id')
    .execute();
  ...
  await db.schema
    .createIndex('interview_hr_connection_id_idx')
    .on('interview')
    .column('hr_connection_id')
    .execute();

  await db.schema
    .createIndex('material_subject_id_idx')
    .on('material')
    .column('subject_id')
    .execute();

  await db.schema
    .createIndex('material_student_user_id_idx')
    .on('material')
    .column('student_user_id')
    .execute();
