...
export class AcceptGiftedSubscriptionUsecase implements UsecaseInterface {
	...
	async execute({
		...
		if (gift.tier.power < giftRecipient.subscription_tier.power) {
			this.logger.debug(
				`Cannot accept a lower tier gift. Gift tier: ${gift.tier.tier} (${gift.tier.power}), current subscription tier: ${giftRecipient.subscription_tier.tier} (${giftRecipient.subscription_tier.power}) for user ${giftRecipient.id}`,
			);
		...
		return {
			...
			activeUntil: new Date(
				activatedGift.activated_at!.getTime() + activatedGift.duration_days * 24 * 60 * 60 * 1000,
			).toISOString(),
