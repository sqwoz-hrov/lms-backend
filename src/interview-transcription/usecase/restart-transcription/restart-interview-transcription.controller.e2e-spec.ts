...
describe('[E2E] Restart interview transcription usecase', () => {
	...
	it.skip('rejects restart when analysis is being retried', () => {
		expect(true).to.equal(
			false,
			'This test is skipped because the current implementation allows restarting transcription even when analysis retry is in progress. This should be fixed in the implementation and then the test should be enabled.',
		);
	});
});
